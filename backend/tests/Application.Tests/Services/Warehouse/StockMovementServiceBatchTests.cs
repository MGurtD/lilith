...
public class StockMovementServiceBatchTests
{
        ...
        bool hasDefaultLocation = true)
    {
        ...
        warehouses.Find(Arg.Any<System.Linq.Expressions.Expression<Func<WarehouseEntity, bool>>>())
        ...
        refsRepo.Find(Arg.Any<System.Linq.Expressions.Expression<Func<Reference, bool>>>())
            .Returns(allRefs);

        var allStocks = existingStocks?.ToList() ?? [];
        stocksRepo.Find(Arg.Any<System.Linq.Expressions.Expression<Func<Stock, bool>>>())
            .Returns(allStocks);
        ...
        return new BatchContext(service, unitOfWork, stocksRepo, stockMovementsRepo, defaultLocationId);
    }
