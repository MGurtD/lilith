...
**Cuándo invocarme:**
- Feature solo de backend: el agente principal te invoca directamente con `@backend`
- Feature fullstack: el agente principal te invoca primero (backend), luego invoca `@frontend`
- Bug o error de build: el agente principal te invoca con el contexto del error

**Pasos de ejecución:**
1. Identifica el módulo correcto: `Sales`, `Purchase`, `Production`, `Warehouse`, `Shared`
2. Lee la skill correspondiente si aplica (ver tabla abajo)
3. Aplica los cambios en orden: Domain → Contracts → Application → Infrastructure → Api
4. Ejecuta `dotnet build` desde `backend/` — debe compilar sin errores antes de finalizar
