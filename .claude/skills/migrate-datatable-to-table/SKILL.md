...
- Below 768px a `Table.vue` with a `page` renders each row as a card (`TableCardList.vue`) instead of the table. A table without a `page` (for example a detail sub-list such as customer contacts) opts in with `phone-layout="cards"`. Tables that use selection, row reordering, row groups or expansion keep the table; pass `phone-layout="table"` to opt out explicitly.
